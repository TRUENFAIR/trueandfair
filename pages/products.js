        ...
        <div className="md:container p-4 mx-auto">
          ...
          <div className="grid md:grid-cols-8 grid-cols-4 md:gap-4 gap-2 pt-12">
            {buttons.map((button, index) => (
              <div key={index} onClick={() => { buttonClick(index) }} className={`${index === buttonIndex ? 'bg-yellow text-zinc-900' : 'bg-zinc-100'} w-full h-16 hover:scale-105 cursor-pointer transition rounded-sm flex items-center justify-center relative p-2`}>
                <p className="text-xs tracking-tight font-medium text-center truncate leading-4 uppercase">{button.name}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid md:grid-cols-4 grid-cols-2 md:gap-12 gap-4 md:pt-12 pt-4">            
            {buttons[buttonIndex].products.map((product, index) => (
              <div key={index} className="w-full h-44 md:h-72 shadow-md shadow-slate-200 hover:scale-105 transition-all text-white font-bold uppercase relative cursor-pointer">
